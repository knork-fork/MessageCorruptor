...
{
    public function corruptMessage(string $message, Array $percentages) : Array
    {
        // Limit message to a certain set of characters
        $message = $this->limitCharacters($message);

        // Check if message empty or contains only spaces
        if ($message == "" || strlen(trim($message)) == 0)
            throw new Exception("Invalid message argument.");
